...
',($U == 'PROCEDURE' ? '<td>'.html_select("fields[$t][inout]", explode('|', $cd), $l['inout']) : ''),'<th>';
        ...
        }echo '<td>',(support('move_col') ? "<input type='image' class='icon' name='add[$t]' src='".h(preg_replace('~\\?.*~', '', ME).'?file=plus.gif&version=4.8.1')."' alt='+' title='".'Add next'."'> "."<input type='image' class='icon' name='up[$t]' src='".h(preg_replace('~\\?.*~', '', ME).'?file=up.gif&version=4.8.1')."' alt='â†‘' title='".'Move up'."'> "."<input type='image' class='icon' name='down[$t]' src='".h(preg_replace('~\\?.*~', '', ME).'?file=down.gif&version=4.8.1')."' alt='â†“' title='".'Move down'."'> " : ''),($_e == '' || support('drop_col') ? "<input type='image' class='icon' name='drop_col[$t]' src='".h(preg_replace('~\\?.*~', '', ME).'?file=cross.gif&version=4.8.1')."' alt='x' title='".'Remove'."'>" : '');
    }
