...
<div class="main-menu menu-fixed {{(($configData['theme'] === 'dark') || ($configData['theme'] === 'semi-dark')) ? 'menu-light' : 'menu-dark'}} site-prefer-black menu-accordion menu-shadow"
    ...
    <div class="main-menu-content scroll-menu">
     {{-- add "navigation-main" class if you need scroll on click and defalut collapse  --}}
        <ul class="navigation  site-prefer-black" id="main-menu-navigation" data-menu="menu-navigation">
            ...
            @endphp
            <li class="module-title nav-item    @if (isset($module))
            @if ('/'.Request::path().'/'.$module->id == $menu->url) {{'active open'}} @else '' @endif
            @endif {{ $custom_classes }} {{Route::currentRouteName() === $menu->slug ? 'active' : ''}} {{'/'.Request::path() == $menu->url ? 'active-url open active':''}}">
                {{-- {{$menu->url}} --}}
                {{-- {{Request::path()}} --}}
                ...
                <a href="{{isset($menu->url)? url($menu->url):'javascript:void(0)'}}" class="d-flex"
                    ...
                    <span class="menu-title text-wrap {{ $course_text }}"
                    @if (isset($menu->moduleComplete) )
                    style= "text-decoration:none !important";
                  @endif
                 onclick="moduleInfo(this)">{{ __($menu->name) }}</span>
                    @if (isset($menu->badge))
                ...
                @include('panels/submenu', ['menu' => $menu])
                @endif

                <!-- @isset($menu->addDivider)
          <div class="divider border-bottom-dark"></div>
        @endisset -->
        @if(isset($menuData[0]->menu[0]['my-couse-navbar']) && !isset($is_teacher))
                @if ($menuData[0]->menu[0]['my-couse-navbar'] === true)
