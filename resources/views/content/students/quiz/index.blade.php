...
{{-- <x-side-bar-nav :module="$module" pageType="quiz" :section="$section" class="active-nav"/>

    <a class="btn btn-sm mb-1" id="menu-btn"><i data-feather='menu'></i></a>
    --}}

{{-- <x-side-bar-nav :modules="$modules" :module="$module" :section="$quiz->section" :quiz="$quiz" class="active-nav" page="quiz"/> --}}
...
<section class="main-content">
    ...
    <div class="d-flex my-container active-cont" id="quiz-layout">
        ...
        <div class="card text-left   @if (empty($scoreArray['attemptScore']) ) card-first-section @endif">
        ...
        @if (!empty($scoreArray['attemptScore']) )
        @if (isset($scoreArray['attemptScore']['final_exam']))
        <div class="card">
            <div class="">
                    ...
                    <div class="module-exam-score-container">
                        <div class="module-exam-score">
                            <div class="col-sm-3 col-md-2 final_quiz_result_circle">
                                <div class="conainer">
                                    <div class="circle_percent" data-percent="{{Helper::roundScorePercentage($scoreArray['attemptScore'][1],$scoreArray['totalQuestions'])}}">
                                        <div class="circle_inner">
                                            <div class="round_per {{$scoreArray['attemptScore']['circle_class']}}"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="final-score">
                            <p>You Scored:<span style="font-weight:bold;">{{$scoreArray['attemptScore'][1]}} out of {{$scoreArray['totalQuestions']}}</span> </p>
                        </div>
                    </div>
        ...
        <div class="card">
            <div class="">
                <div class="row attempt-container">
                    ...
                    @foreach ($scoreArray['attemptScore'] as $attempt => $score)
                    <div class="attempt text-black font-s">
                        Attempt {{ $attempt }} : <span style="font-weight:bold;">{{ $score }} of {{ $scoreArray['totalQuestions'] }} correct</span> <br>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    @if (!$scoreArray['finalExamCompleted'])
    @php
    ...
    @endphp
    @if($miniQuiz && $showQuizButton )
        <div class="card-footer section-controls">
            <button class="start-quiz btn btn-primary"><span class="btn-txt">{{$quizMessageText}}</span></button>
        </div>
    @elseif(!$miniQuiz)
        <div class="card-footer section-controls">
        <button class="start-quiz btn btn-primary"><span class="btn-txt">{{$quizMessageText}}</span></button>
    ...
    @else
    @if(!empty($quiz->section->nextSection()))
            <div class="card-footer section-controls">
                <a href="{{  route('this-section',$quiz->section->nextSection()) }}" class="btn btn-primary start-quiz-1"><span class="btn-txt">Next</span></a>
            </div>
    @else
            <div class="card-footer section-controls">
                <a class="btn btn-primary start-quiz-1" href="{{  route('my-courses',$quiz->section->module->nextModuleForStudent()) }}"><span class="btn-txt">Next</span></a>
            </div>
    @endif

            
    @endif
    @elseif ($scoreArray['nextSectionExist'])
      <div class="card-footer section-controls">
          <a href="{{$scoreArray['nextSectionExist']}}" class="btn btn-primary start-quiz-1"><span class="btn-txt">Next</span></a>
      </div>
    @else
            <div class="card-footer section-controls">
                <a class="btn btn-primary start-quiz-1" href="{{  route('my-courses',$quiz->section->module->nextModuleForStudent()) }}"><span class="btn-txt">Next</span></a>
...
<script>
    $('.start-quiz').click((e) => {
        e.preventDefault();
        let content = `@if (!$scoreArray['finalExamCompleted'])<x-quiz-questions :quiz=$quiz />@endif`
        $('.start-quiz').hide();
        if (content) {
